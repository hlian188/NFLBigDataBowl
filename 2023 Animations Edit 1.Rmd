...
example_play <- df_plays %>%
                select(gameId, playId, playDescription) %>% filter(gameId == the_game, playId == the_play) 
...
#%>% filter(pff_role %in% c("Pass", "Pass Block", "Pass Rush"))

```

```{r}
#temp <- df_pffScouting %>% select(pff_role, gameId, playId,nflId) %>% filter(gameId == the_game, playId == the_play)



df <- data.frame("force" = rep(seq(2,10,2), 1000))
df <- df[1:nrow(example_play),]

example_play$force <- df
...
at_catch_plot <- ggplot() +
  ...
  geom_point(data = example_play,
   ...
   #geom_segment(data = example_play, #alpha = 0.75,
    #           aes(y = y, x = x, xend = x + force, yend = y,
