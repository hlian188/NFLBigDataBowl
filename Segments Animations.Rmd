...
# force
df_merged <- read_csv("Data/forces_week1.csv",
                    col_types = cols())
...
test <- example_play %>%  filter(frameId == "1") %>% filter(pff_role == "Pass Block")
...
at_catch_plot <- ggplot() +
  ...
  geom_segment(aes(x = 20, y = topLine, xend = 60, yend = topLine), size = 1) + 
  geom_segment(aes(x = 20, y = bottomLine, xend = 60, yend = bottomLine), size = 1) + 
